...
Для работы с R2 в GitHub Actions учетные данные передаются через переменные окружения AWS_ACCESS_KEY_ID и AWS_SECRET_ACCESS_KEY, которые устанавливаются в каждом шаге, где требуется доступ к R2.
